 ...
 */
static constexpr Key<uint64_t, ConfigMutability::RO> device_total_mem_size{"GPU_DEVICE_TOTAL_MEM_SIZE"};

/**
 * @brief Metric to get microarchitecture identifier in major.minor.revision format
 */
static constexpr Key<std::string, ConfigMutability::RO> uarch_version{"GPU_UARCH_VERSION"};

/**
 * @brief Metric to get count of execution units for current GPU
 */
static constexpr Key<int32_t, ConfigMutability::RO> execution_units_count{"GPU_EXECUTION_UNITS_COUNT"};
 ...
 */
static constexpr Key<std::map<std::string, uint64_t>, ConfigMutability::RO> memory_statistics{"GPU_MEMORY_STATISTICS"};

/**
 * @brief Metric to get maximum batch size which does not cause performance degradation due to memory swap impact.
 */
static constexpr Key<uint32_t, ConfigMutability::RO> max_batch_size{"GPU_MAX_BATCH_SIZE"};
 ...
 */
static constexpr Key<bool, ConfigMutability::RO> hw_matmul{"GPU_HW_MATMUL"};
...
static constexpr Key<bool> enable_loop_unrolling{"GPU_ENABLE_LOOP_UNROLLING"};
 ...
 */
static constexpr Key<uint32_t> queue_throttle{"GPU_QUEUE_THROTTLE"};
 ...
 */
static constexpr Key<uint32_t> queue_priority{"GPU_QUEUE_PRIORITY"};
 ...
 */
static constexpr Key<HostTaskPriority> host_task_priority{"GPU_HOST_TASK_PRIORITY"};
}  // namespace hint
