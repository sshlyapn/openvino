...
struct memory {
    ...
    // Device <== Host
    virtual event::ptr copy_from(stream& stream, const void* data_ptr, size_t src_offset, size_t dst_offset, size_t size, bool blocking) = 0;
    ...
    // Device ==> Host
    virtual event::ptr copy_to(stream& stream, void* data_ptr, size_t src_offset, size_t dst_offset, size_t size, bool blocking) const = 0;
    ...
    virtual event::ptr copy_from(stream& stream, const void* src_ptr, size_t size = 0, bool blocking = true) {
        const auto zero_offset = 0;
        const auto data_size = size == 0 ? _bytes_count : size;
...
struct simple_attached_memory : memory {
    ...
    virtual event::ptr copy_from(stream& stream, const void* data_ptr, size_t src_offset, size_t dst_offset, size_t size, bool blocking) {
        OPENVINO_NOT_IMPLEMENTED;
    }
    virtual event::ptr copy_from(stream& stream, const memory& src_mem, size_t src_offset, size_t dst_offset, size_t size, bool blocking) {
        OPENVINO_NOT_IMPLEMENTED;
    }
    virtual event::ptr copy_to(stream& stream, void* data_ptr, size_t src_offset, size_t dst_offset, size_t size, bool blocking) const {
        OPENVINO_NOT_IMPLEMENTED;
    }
