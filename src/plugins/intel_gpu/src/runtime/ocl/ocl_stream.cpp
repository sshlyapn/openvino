...
cl_int set_kernel_arg(ocl_kernel_type& kernel, uint32_t idx, cldnn::memory::cptr mem) {
    ...
    if (mem->get_layout().format.is_image_2d()) {
        ...
        GPU_DEBUG_TRACE_DETAIL << "kernel: " << kernel.get() << " set arg (" << std::dynamic_pointer_cast<const ocl::gpu_usm>(mem)->get_allocation_type() << ") " << idx << " mem: " << buf.get() << " size: " << mem->size() << std::endl;
