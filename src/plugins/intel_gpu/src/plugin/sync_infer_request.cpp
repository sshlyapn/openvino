...
void SyncInferRequest::allocate_states() {
    ...
    for (auto& vi : variables_info) {
        ...
        if (compressed) {
            m_variables.emplace(vi.first, std::make_shared<VariableStateIndirectKVCacheCompressed>(vi.second, m_context, m_shape_predictor, states_layouts, beam_axis, concat_axis, has_zp_state));
