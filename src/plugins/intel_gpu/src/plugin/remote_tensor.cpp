...
struct mem_wrapper {
    mem_wrapper(cldnn::stream& stream, cldnn::memory_ptr mem_ptr, void* data_ptr)
        ...
        , _mem_ptr(mem_ptr)
        , _data_ptr(data_ptr) {
            OPENVINO_ASSERT((_data_ptr != nullptr) != (_mem_ptr != nullptr), "[GPU] Invalid memory configuration");
        }

    void copy_to(mem_wrapper& dst, size_t src_offset, size_t dst_offset, size_t size) const {
        if (_data_ptr != nullptr) {
            OPENVINO_ASSERT(dst._mem_ptr, "[GPU] Unexpected host to host copy call for Remote Tensors");

            // Device <== Host
            dst._mem_ptr->copy_from(_stream, _data_ptr, src_offset, dst_offset, size, true);
        } else {
            if (dst._data_ptr != nullptr) {
                // Device ==> Host
                _mem_ptr->copy_to(_stream, dst._data_ptr, src_offset, dst_offset, size, true);
            } else {
                // Device ==> Device
                _mem_ptr->copy_to(_stream, *dst._mem_ptr, src_offset, dst_offset, size, true);
            }
        }
    }
...
private:
    ...
    cldnn::memory_ptr _mem_ptr = nullptr;
