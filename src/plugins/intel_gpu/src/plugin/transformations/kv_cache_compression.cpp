...
KVCacheCompressionMatcher::KVCacheCompressionMatcher() {
    ...
    int USE_KV_CACHE_COMP = 0;
    if (const auto env_var = std::getenv("USE_KV_CACHE_COMP")) {
        std::istringstream ss(env_var);
        ss >> USE_KV_CACHE_COMP;
    }

    if (USE_KV_CACHE_COMP == 0) {
    ...
    int USE_PER_HEAD_COMP = 1;
    if (const auto env_var = std::getenv("USE_PER_HEAD_COMP")) {
        std::istringstream ss(env_var);
        ss >> USE_PER_HEAD_COMP;
    }

    int USE_ASYM_COMP = 1;
    if (const auto env_var = std::getenv("USE_ASYM_COMP")) {
        std::istringstream ss(env_var);
        ss >> USE_ASYM_COMP;
    }

    int USE_ZP_INPUT = 0;
    if (const auto env_var = std::getenv("USE_ZP_INPUT")) {
        std::istringstream ss(env_var);
        ss >> USE_ZP_INPUT;
    }
    ...
    if (USE_ZP_INPUT && combine_scales_and_zp) {
        combine_scales_and_zp = false;
    }

    if (USE_KV_CACHE_COMP != 0 && USE_PER_HEAD_COMP != 0 && USE_ASYM_COMP != 0) {
        std::cout << "--> KV-cache compression configuration: "
                  << "DEFAULT ("
                  << "USE_PER_HEAD_COMP = " << USE_PER_HEAD_COMP << ", "
                  << "USE_ASYM_COMP = " << USE_ASYM_COMP << ", "
                  << "USE_ZP_INPUT = " << USE_ZP_INPUT << ")\n\n";
    } else {
        std::cout << "--> KV-cache compression configuration: "
                  << "CUSTOM ("
                  << "USE_PER_HEAD_COMP = " << USE_PER_HEAD_COMP << ", "
