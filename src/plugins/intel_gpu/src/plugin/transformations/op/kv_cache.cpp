...
void KVCache::validate_and_infer_types() {
    ...
    if (m_compressed) {
        ...
        if (m_quantization_config.is_asymmetric_quantization() && !m_combine_scales_and_zp) {
            set_output_type(out_ports++, m_quantization_config.scale_dt, shapes[3]);
        }
    }
}
...
std::vector<ov::PartialShape> shape_infer(const KVCache* op,
    ...
    if (op->get_output_size() >= 3) {
        ...
        auto quantized_data_shapes = ov::intel_gpu::op::DynamicQuantize::shape_infer(&op, {input_shapes[1]}, config, scales_zp_output_order, combine_scales_and_zp);

        std::cout << "Base shapes: " << out_shapes[0] << " " << out_shapes[1] << "\n";
        std::cout << "Start shape: " << input_shapes[3] << ". Qunatized shapes: " << quantized_data_shapes[0] << " " << quantized_data_shapes[1]
                  << " (total quant shapes: " << quantized_data_shapes.size() << ", input: " << input_shapes.size() << ", out_shapes=" << out_shapes.size() << ")\n";
