...
void remove_redundant_reorders::run(program& p) {
    ...
    while (itr != p.get_processing_order().end()) {
        auto& node_ptr = *itr++;
        if (!node_ptr->is_type<reorder>() || !node_ptr->is_in_data_flow() || node_ptr->get_users().size() != 1 || !node_ptr->get_users().front()->is_type<shape_of>())
            continue;

        auto& node = node_ptr->as<reorder>();
        auto& dep = node_ptr->get_dependency(0);

        std::cout << "Remove reorder before shape_of: " << node.id() << " (" << dep.id() << ")" << std::endl;;
        LOG_NODE_REMOVAL(node.id());
        p.replace_all_usages(node, dep);
        p.add_optimized_primitive_info(node.id());
        p.remove_all_connections(node);
