...
void prepare_primitive_fusing::fuse_simple_primitives(program &p) {
    ...
    while (itr != p.get_processing_order().end()) {
        ...
        auto try_fuse_through = [&](program_node& node) -> std::vector<program_node*> {
            ...
            // 3. Paddings not allowed for fused node
        ...
        auto fuse_activation_f = [&](activation_node& activation_node) {
            auto fuse_through = try_fuse_through(activation_node.get_dependency(0));
            bool use_fuse_through = fuse_through.size() > 1;

            // Extract fused_node from vector
            auto& input_data = *fuse_through.back();
            fuse_through.pop_back();

            if (activation_node.get_dependencies().size() >= 3 ||
                (use_fuse_through && activation_node.get_dependencies().size() > 1))
                return;
            ...
            p.fuse_nodes(input_data, activation_node, &fusing_history, fuse_through);
        };
        ...
        auto fuse_quantize_f = [&](quantize_node& quantize_node) {
            auto fuse_through = try_fuse_through(quantize_node.get_dependency(0));
            bool use_fuse_through = fuse_through.size() > 1;
            ...
            p.fuse_nodes(input_data, quantize_node, &fusing_history, fuse_through);
        };
        ...
        auto fuse_eltwise_f = [&](eltwise_node& node) {
            ...
            const size_t parents_num = 2;
            std::vector<program_node*> pass_through[parents_num];

            for (size_t i = 0; i < parents.size(); i++) {
                size_t second_input_idx = i ^ 1;
                auto fuse_through = try_fuse_through(*parents[i]);
                bool use_fuse_through = fuse_through.size() > 1;

                // Extract fused_node from vector
                parents[i] = fuse_through.back();
                fuse_through.pop_back();
                ...
                if (use_fuse_through) {
                    ...
                    pass_through[i] = std::move(fuse_through);
                }
            }

            // Fallback to a classic path if fusion through logic can't be applied
            for (size_t i = 0; i < parents.size(); i++) {
                if (!pass_through[i].empty() && !can_fuse_parents[i]) {
                    parents[i] = pass_through[i].front();
                    pass_through[i].clear();
                }
            }
