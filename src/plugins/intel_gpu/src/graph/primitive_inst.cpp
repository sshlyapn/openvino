        ...
        if (!cached_impl) {
            if (_dynamic_impl) {
                ...
                compilation_context.push_task(updated_params.hash(), [this, &compilation_context, updated_params]() {
                    ...
                    // TODO: try to combine weights reorder's and primitive's kernels compilation in single request
                    ...
                    if (reorder_kernel_params && !cache.has(*reorder_kernel_params)) {
                        ...
                        auto kernels = _program->get_kernels_cache().compile(*reorder_kernel_params, reorder_impl->get_kernels_source());
...
event::ptr primitive_inst::update_weights() {
    ...
    if (reorder_kernel_params) {
        auto weights_memory = _reordered_weights_cache.get(expected_layout);
        if (weights_memory) {
            _weights_memory = weights_memory;
        ...
        auto result = _node->weights_reorderer.execute_task(expected_layout.format, [&]() {
            ...
            if (auto cached_impl = cache.get(*reorder_kernel_params)) {
                ...
                reorder_inst->set_impl(std::move(reorder_impl->clone()));
            }
            ...
            return std::make_pair(weights_memory, ev);
        });
