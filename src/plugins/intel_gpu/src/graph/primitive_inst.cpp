...
void primitive_inst::realloc_if_needed(bool prev_execution_skipped) {
    ...
    // intermediate memory allocation is required for primitives consisting of multiple kernels in dynamic case
    {
        ...
        for (size_t i = 0; i < ibuf_layouts.size(); ++i) {
            ...
            bool can_reuse = i < _intermediates_memory.size() &&
                             ibuf_layouts[i].bytes_count() <= max_intermediates_memory_sizes[i];
            if (can_reuse) {
                auto allocation_type = _intermediates_memory[i]->get_allocation_type();
                can_reuse = (need_lockable && allocation_type != cldnn::allocation_type::usm_device) ||
                            (!need_lockable && allocation_type != cldnn::allocation_type::usm_host);
            }
