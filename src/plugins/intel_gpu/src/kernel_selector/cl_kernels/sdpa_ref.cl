...
)
{
    ...
    if (get_local_id(2) == 0) {
        ...
        if (SOURCE_SEQ_LEN == 9 && b0 == 2) {
            uint key_offset = FUNC_CALL(get_input1_index)(OPTIONAL_SHAPE_INFO_TENSOR b_idx, b1, 0, 0, s, h);
            // printf("Key original idx: %d %d %d %d, h=%d. Updated b0 index in beam_table = %d. bb=%d. Key[%d]=%f (new)\n",
            //     b0, b1, target_seq_idx, s, h, FUNC_CALL(get_bt_index_key)(OPTIONAL_SHAPE_INFO_TENSOR b0, b1, 0, 0, s, h), b_idx, key_offset, key_input[key_offset]);
        }
#else
                uint b_idx = b0;
#endif
    ...
    for (uint s = 0; s < SOURCE_SEQ_LEN /* seq_len */; s++) {
        ...
        if (SOURCE_SEQ_LEN == 9 && b0 == 2) {
            ...
            // printf("Value original idx: %d %d %d %d. Updated b0 index in beam_table = %d. bb=%d Val[%d]=%f\n", b0, b1, s, head_size_idx, FUNC_CALL(get_bt_index_value)(OPTIONAL_SHAPE_INFO_TENSOR b0, b1, 0, 0, s, head_size_idx), b_idx, value_offset, value_input[value_offset]);
        }
#else
        uint b_idx = b0;
#endif
