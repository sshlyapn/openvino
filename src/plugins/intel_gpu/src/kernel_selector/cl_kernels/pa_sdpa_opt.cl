...
KERNEL(pa_sdpa_opt)(
...

    {
        ...
        for (uint block_num = 0; block_num < blocks_num; block_num++) {
            const uint block_offset = block_indices[start_block_idx + block_num * SUBGROUPS_PER_WG] * HEAD_SIZE * HEADS_NUM * SUBGROUP_SIZE + head_num_idx * HEAD_SIZE * SUBGROUP_SIZE;
...

    {
        ...
        for (uint block_num = 0; block_num < blocks_num_per_partition; block_num++) {
            const uint block_offset = block_indices[start_block_idx + block_num] * HEADS_NUM * HEAD_SIZE * PAGED_ATTENTION_BLOCK_SIZE + head_num_idx * HEAD_SIZE * PAGED_ATTENTION_BLOCK_SIZE + sgid * SUBGROUP_SIZE;
        ...
        if (leftovers != 0) {
            ...
            const uint block_offset = block_indices[last_block_idx] * HEAD_SIZE * HEADS_NUM * PAGED_ATTENTION_BLOCK_SIZE + head_num_idx * HEAD_SIZE * PAGED_ATTENTION_BLOCK_SIZE + sgid * SUBGROUP_SIZE;
