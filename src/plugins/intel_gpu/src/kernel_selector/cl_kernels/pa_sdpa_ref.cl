...
KERNEL(pa_sdpa_ref)(
...

    {
        ...
        for (uint block_num = 0; block_num < blocks_num; block_num++) {
            ...
            for (uint hs = 0; hs < Q_LOAD_ITERS; hs++) {
                for (uint qk_idx = 0; qk_idx < QK_VALS_PER_SG_PER_ITER; qk_idx++) {
                    ...
                    OUTPUT_TYPE qk_debug = qk[qk_idx];
                    qk[qk_idx] = mad(q[hs], k, qk[qk_idx]);

                    // if (get_global_id(0) == 0 && get_global_id(1) == 0 && get_sub_group_id() == 0 && context_len == 571 && qk_idx == 0 && get_group_id(2) == 0)
                    //     printf("%d %d %d %d %d. %f = %f * %f + %f\n", get_global_id(2), block_num, hs, sglid, qk_idx, qk[qk_idx], q[hs], k, qk_debug);
                }
            }
        ...
        // if (get_global_id(0) == 0 && get_global_id(1) == 0 && get_local_id(2) == 0 && context_len >= 570)
        //     printf("%d. %d. SDPA kernel GEMM1: %d; qk_max=%f (token=%d), blocks_num=%d, total_blocks_num=%d, portion_id=%d, num_of_portions=%d, scale = %f\n",
        //         context_len, get_global_id(2), (uint)total_time, qk_max, token_idx_debug, blocks_num, total_blocks_num, portion_id, num_of_portions, scale[0]);
    }
...
KERNEL(pa_sdpa_finalization_stage)(
    ...
    if (total_num_of_portions == 1) {
        ...
        // if (token_idx < 2 && head_dim == 0) {
        //     printf("%d %d %d. context_len = %d, num_of_portions = %d, total_num_of_portions = %d, iters_num = %d, global_max = %f, global_sum = %f. Acc=%f\n",
        //         batch_idx, token_idx, head_dim, context_lens[batch_idx], num_of_portions, total_num_of_portions, iters_num, global_max, global_sum, acc);
        // }
