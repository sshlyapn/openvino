...
KERNEL(fc)(
...
#if USE_SLM && !USE_SLM_FOR_SCALES
    __local ACCUMULATOR_TYPE wei_local[TILE_IFM * SIMD * TILE_OFM * SIMD];
    __local ACCUMULATOR_TYPE wei_local_swap[TILE_IFM * SIMD * TILE_OFM * SIMD];
#endif
    ...
    // __local ACCUMULATOR_TYPE wei_local[TILE_IFM * SIMD * TILE_OFM * SIMD];
    // __local ACCUMULATOR_TYPE wei_local_swap[TILE_IFM * SIMD * TILE_OFM * SIMD];
#elif USE_SLM_FOR_SCALES && DECOMPRESSION_SCALE_GROUPS_NUM
    #if DECOMPRESSION_SCALE_GROUPS_NUM != 32 || TILE_OFM != 2
        #error "Experimental support of 32 DECOMPRESSION_SCALE_GROUPS_NUM only and TILE_OFM == 2"
    #endif

    __local MAKE_VECTOR_TYPE(ACTIVATION_TYPE, TILE_OFM) scales_local_mem[SIMD * DECOMPRESSION_SCALE_GROUPS_NUM]; // store TILE_OFM * SIMD * DECOMPRESSION_SCALE_GROUPS_NUM

    #define LOADS_PER_SG 8

    uint scales_idx = out_f * DECOMPRESSION_SCALE_GROUPS_NUM * SIMD * TILE_OFM + local_id * SIMD * TILE_OFM * LOADS_PER_SG;
    uint wei_local_idx = local_id * SIMD * LOADS_PER_SG + sglid;
    ACCUMULATOR_VEC_TYPE d_scale = BLOCK_READN(ACCUMULATOR_TYPE, TILE_OFM, decompression_scale, scales_idx);
    scales_local_mem[wei_local_idx] = d_scale;
    wei_local_idx += SIMD;

    scales_idx += TILE_OFM * SIMD;
    d_scale = BLOCK_READN(ACCUMULATOR_TYPE, TILE_OFM, decompression_scale, scales_idx);
    scales_local_mem[wei_local_idx] = d_scale;
    wei_local_idx += SIMD;
    ...
    #if USE_SLM && COMPRESSED_WEIGHTS_INT4 && USE_WEIGHTS_PREFETCH && !USE_SLM_FOR_SCALES
    {
        #if TILE_OFM != 2
    ...
    for (uint ni = 0; ni < iterations; ++ni) {
        ...
        //       It doesn't also show noticable performance improvement on tested configurations.
        ...
        #if USE_SLM && COMPRESSED_WEIGHTS_INT4 && !USE_SLM_FOR_SCALES
            #if TILE_OFM != 2
            #error "Can not use slm with TILE_OFM != 2"
            #endif
            ...
            __local LOCAL_FILTER_VEC2* wei2_local_store;
            __local LOCAL_FILTER_VEC2* wei2_local;

#if USE_WEIGHTS_PREFETCH
            if (ni % 2 == 0) {
                wei2_local = (__local LOCAL_FILTER_VEC2*)&wei_local;
                wei2_local_store = (__local LOCAL_FILTER_VEC2*)&wei_local_swap;
            } else {
                wei2_local = (__local LOCAL_FILTER_VEC2*)&wei_local_swap;
                wei2_local_store = (__local LOCAL_FILTER_VEC2*)&wei_local;
            }
#else
            wei2_local = (__local LOCAL_FILTER_VEC2*)&wei_local;
            wei2_local_store = (__local LOCAL_FILTER_VEC2*)&wei_local;
            ...
            unroll_for(uint load_iter = 0; load_iter < WEIGHTS_PRELOAD_ITERS; ++load_iter) {
                ...
                wei2_local_store[wei_local_idx] = wei_tmp.s01;
                wei_local_idx += SIMD;
                wei2_local_store[wei_local_idx] = wei_tmp.s23;
                wei_local_idx += SIMD;
                weights_idx += SIMD * LOAD_SIZE_PACKED / 2;
            }
            ...
            #define LOAD_SIZE 2
            #define LOCAL_FILTER_VEC_TYPE MAKE_VECTOR_TYPE(ACCUMULATOR_TYPE, LOAD_SIZE)
            #define TO_LOCAL_FILTER_VEC_TYPE(x) CAT(convert_, LOCAL_FILTER_VEC_TYPE)(x)
            #define LOCAL_FILTER_BLOCK_READ(ptr, offset)       BLOCK_READN(FILTER_TYPE, LOAD_SIZE, ptr, offset)
            ...
            uint weights_idx = weights_offset + local_id * SIMD * LOAD_SIZE * WEIGHTS_PRELOAD_ITERS;
            uint wei_local_idx = local_id * SIMD * WEIGHTS_PRELOAD_ITERS + sglid; // store by two fp16 values
            unroll_for(uint load_iter = 0; load_iter < WEIGHTS_PRELOAD_ITERS; ++load_iter) {
                LOCAL_FILTER_VEC_TYPE wei_tmp;
                ...
                wei_tmp = TO_LOCAL_FILTER_VEC_TYPE(LOCAL_FILTER_BLOCK_READ(weights, weights_idx));
                    ...
                    const uint ki = local_id; // TODO: Need to calculate it
                    ACCUMULATOR_TYPE* w = (ACCUMULATOR_TYPE*)(&wei_tmp);
                    unroll_for(uint fi = 0; fi < LOAD_SIZE; ++fi) {
                        // const uint w_idx = kii * TILE_OFM + fi;
        ...
        // Dynamic shape will be aligned to 64, so ignore it
        #ifdef BATCH_LEFTOVERS && !IS_DYNAMIC
            if (get_global_id(2) >= BATCH_LEFTOVERS) {
                weights_offset += TILE_K_OFM_PACKED * SIMD * TILE_IFM * SIMD / TILE_K;
                continue;
            }
        #endif
...
#if DECOMPRESSION_SCALE_POST_OP
#if USE_SLM_FOR_SCALES && DECOMPRESSION_SCALE_GROUPS_NUM
        const uint scale_offset = ((ni*TILE_IFM*SIMD) / DECOMPRESSION_SCALE_GROUP_SIZE) * SIMD + sglid;
        MAKE_VECTOR_TYPE(ACTIVATION_TYPE, TILE_OFM) ds = scales_local_mem[scale_offset];
        unroll_for (uint bi = 0; bi < TILE_B; ++bi) {
            unroll_for(uint fi = 0; fi < TILE_OFM; ++fi) {
                ((ACCUMULATOR_TYPE*)(&acc[bi]))[fi] += ((ACCUMULATOR_TYPE*)(&acc_tmp[bi]))[fi] * ds[fi];
            }
        }
    ...
    #define LEFTOVER_IFM               (MAIN_LOOP_ELEMENTS_COUNT % (TILE_IFM * SIMD))
    {
        ...
        unroll_for(uint ki = 0; ki < CEIL_DIV(LEFTOVER_IFM, TILE_K); ++ki) {
            ...
            printf("FC bf_tiled kernel: Leftovers processing!\n");
