...
inline void FUNC(fc_bf_tiled_kernel_tile_b1)(
    ...
    uint out_b = ((batch_mega_block * DISPATCH_BSV + batch_mini_block) * LOCAL_TILE_B);

    ACCUMULATOR_VEC_TYPE acc[LOCAL_TILE_B] = { };
    INPUT_VEC_TYPE       in_0[LOCAL_TILE_B] = { };
    ...
    {
        INPUT0_TYPE tmp_input = input[input_offset + get_sub_group_local_id() % LOCAL_TILE_B * TILE_IN_B_PITCH];
        ...
        #endif
        unroll_for(uint bi = 0; bi < LOCAL_TILE_B; ++bi) {
    ...
    for (uint ni = 0; ni < iterations; ++ni) {
        ...
        CONST_LOOP(LOCAL_TILE_B, LOAD_IN_0);
        #undef LOAD_IN_0
        input_offset += TILE_IFM * SIMD - TILE_IN_B_PITCH * LOCAL_TILE_B;
        ...
        //       It doesn't also show noticable performance improvement on tested configurations.
            ...
            ACCUMULATOR_VEC_TYPE acc_tmp[LOCAL_TILE_B] = { };
        #endif
    ...
    ACTIVATION_VEC_TYPE activated[LOCAL_TILE_B] = { };
    for (uint bi = 0; bi < LOCAL_TILE_B; ++bi) {
    ...
    OUTPUT_VEC_TYPE result[LOCAL_TILE_B] = { };
#if HAS_FUSED_OPS
    unroll_for (uint bi = 0; bi < LOCAL_TILE_B; ++bi) {
    #if TILE_OFM > 1
        unroll_for(uint fi = 0; fi < TILE_OFM; ++fi) {
...
#endif
        CONST_LOOP(LOCAL_TILE_B, WRITE_OUTPUT);
        #undef WRITE_OUTPUT
    } else {
        output_offset += sglid;
