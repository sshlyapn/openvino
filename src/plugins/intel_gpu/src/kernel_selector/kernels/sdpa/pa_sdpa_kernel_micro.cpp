...
std::mutex PagedAttentionSDPAKernelMicro::m;

void PagedAttentionSDPAKernelMicro::init_microkernels(const sdpa_params& params, micro::Package& gemm_kq, micro::Package& gemm_vs, bool is_prefill) const {
...
ParamsKey PagedAttentionSDPAKernelMicro::GetSupportedKey() const {
    ParamsKey k;
...
bool PagedAttentionSDPAKernelMicro::Validate(const Params& p) const {
...
JitConstants PagedAttentionSDPAKernelMicro::GetJitConstants(const sdpa_params& params, const micro::Package& gemm_kq, const micro::Package& gemm_vs) const {
    ...
    if (params.conf.is_paged_attention) {
        jit.AddConstant(MakeJitConstant("PAGE_SIZE", K.X().v));
        jit.AddConstant(MakeJitConstant("WITH_PAGED_ATTN", 1));
        jit.AddConstant(MakeJitConstant("INDEX_DATA_T", toCLType(params.inputs[3].GetDType())));
    }
...
CommonDispatchData PagedAttentionSDPAKernelMicro::SetDefault(const sdpa_params& params, const micro::Package& gemm_kq, const micro::Package& gemm_vs) const {
    ...
    auto subsequences_num = params.inputs[3].Batch().v;

    dispatch_data.lws = {subgroup_size(params.engineInfo.arch), (size_t)sg_per_wg, 1};
    dispatch_data.gws = dispatch_data.lws;

    dispatch_data.gws[0] *= CeilDiv(params.inputs[0].Batch().v, wg_tile_q);
    dispatch_data.gws[1] *= subsequences_num;

    return dispatch_data;
}

clKernelData PagedAttentionSDPAKernelMicro::get_kernel_data(const sdpa_params& params, bool is_prefill) const {
...
KernelsData PagedAttentionSDPAKernelMicro::GetKernelsData(const Params& params) const {
...
void PagedAttentionSDPAKernelMicro::GetUpdateDispatchDataFunc(KernelData& kd) const {
...
KernelsPriority PagedAttentionSDPAKernelMicro::GetKernelsPriority(const Params& /*params*/) const {
