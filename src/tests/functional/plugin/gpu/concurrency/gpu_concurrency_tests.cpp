...
TEST(OVConcurrencyTest, canSwapTensorsBetweenIRs) {
    std::vector<std::vector<uint8_t>> ref;
    ...
    auto calc_ref_results = [&](const ov::Tensor& tensor){
        const auto tensorSize = tensor.get_byte_size();
        const auto inBlobBuf = static_cast<uint8_t*>(tensor.data());
        std::vector<uint8_t> inData(inBlobBuf, inBlobBuf + tensorSize);
        auto reOutData = ngraph::helpers::interpreterFunction(fn, {inData}).front().second;
        ref.push_back(reOutData);
    };
