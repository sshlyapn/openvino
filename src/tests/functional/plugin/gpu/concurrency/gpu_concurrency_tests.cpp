...
TEST(canSwapTensorsBetweenIRs, inputsDifferentContexts) {
    auto ie = ov::Core();
    ...
    auto compiled_model1 = ie.compile_model(fn, gpuDeviceFirst);
    auto compiled_model2 = ie.compile_model(fn, gpuDeviceSecond);
...
TEST(canSwapTensorsBetweenIRs, outputsDifferentContexts) {
    auto ie = ov::Core();
    std::vector<std::string> gpuDevices;
    std::vector<std::string> availableDevices = ie.get_available_devices();
