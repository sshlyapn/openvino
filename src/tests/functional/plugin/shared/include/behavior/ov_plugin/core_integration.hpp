...
#include <openvino/runtime/intel_gpu/config.hpp>
...
#define ASSERT_METRIC_SUPPORTED(metricKey)                                                      \
{                                                                                               \
    auto metrics = ie.get_metric(deviceName, ov::supported_metrics);                       \
    auto it = std::find(metrics.begin(), metrics.end(), metricKey.str());                       \
...
inline bool supportsAvaliableDevices(ov::runtime::Core& ie, const std::string& deviceName) {
    auto supportedMetricKeys = ie.get_metric(deviceName, ov::supported_metrics);
    return supportedMetricKeys.end() !=
           std::find(std::begin(supportedMetricKeys), std::end(supportedMetricKeys), ov::available_devices.str());
}

bool supportsDeviceID(ov::runtime::Core& ie, const std::string& deviceName) {
    auto supportedConfigKeys =
            ie.get_metric(deviceName, ov::supported_config_keys);
    return supportedConfigKeys.end() !=
           std::find(std::begin(supportedConfigKeys), std::end(supportedConfigKeys), ov::device::id.str());
}
...
TEST_P(OVClassBasicTestP, registerNewPluginNoThrows) {
    ...
    OV_ASSERT_NO_THROW(ie.get_metric("NEW_DEVICE_NAME", ov::supported_config_keys));
}
...
TEST_P(OVClassBasicTestP, SetConfigAllThrows) {
    ov::runtime::Core ie = createCoreWithTemplate();
    OV_ASSERT_NO_THROW(ie.set_config({{"unsupported_key", "4"}}));
    ASSERT_ANY_THROW(ie.get_versions(deviceName));
}

TEST_P(OVClassBasicTestP, SetConfigForUnRegisteredDeviceThrows) {
    ov::runtime::Core ie = createCoreWithTemplate();
    ASSERT_THROW(ie.set_config("unregistered_device", {{"unsupported_key", "4"}}), ov::Exception);
}

TEST_P(OVClassBasicTestP, SetConfigNoThrow) {
    ov::runtime::Core ie = createCoreWithTemplate();
    OV_ASSERT_NO_THROW(ie.set_config(deviceName, ov::enable_profiling(true)));
}

TEST_P(OVClassBasicTestP, SetConfigAllNoThrow) {
    ov::runtime::Core ie = createCoreWithTemplate();
    OV_ASSERT_NO_THROW(ie.set_config(ov::enable_profiling(true)));
    OV_ASSERT_NO_THROW(ie.get_versions(deviceName));
}

TEST(OVClassBasicTest, smoke_SetConfigHeteroThrows) {
    ov::runtime::Core ie = createCoreWithTemplate();
    OV_ASSERT_NO_THROW(ie.set_config(CommonTestUtils::DEVICE_HETERO, ov::enable_profiling(true)));
}

TEST_P(OVClassBasicTestP, SetConfigHeteroTargetFallbackThrows) {
    ov::runtime::Core ie = createCoreWithTemplate();
    OV_ASSERT_NO_THROW(ie.set_config(CommonTestUtils::DEVICE_HETERO, ov::target_fallback(deviceName)));
}
...
TEST(OVClassBasicTest, smoke_SetConfigHeteroNoThrow) {
    ...
    OV_ASSERT_NO_THROW(ie.set_config(CommonTestUtils::DEVICE_HETERO, ov::dump_graph_dot(true)));
    OV_ASSERT_NO_THROW(value = ie.get_config(CommonTestUtils::DEVICE_HETERO, ov::dump_graph_dot));
    ASSERT_TRUE(value);

    OV_ASSERT_NO_THROW(ie.set_config(CommonTestUtils::DEVICE_HETERO, ov::dump_graph_dot(false)));
    OV_ASSERT_NO_THROW(value = ie.get_config(CommonTestUtils::DEVICE_HETERO, ov::dump_graph_dot));
    ASSERT_FALSE(value);
}
...
TEST_P(OVClassSpecificDeviceTestSetConfig, SetConfigSpecificDeviceNoThrow) {
    ...
    auto deviceIDs = ie.get_metric(clearDeviceName, ov::available_devices);
    ...
    OV_ASSERT_NO_THROW(ie.set_config(deviceName, ov::enable_profiling(true)));
    bool value = false;
    OV_ASSERT_NO_THROW(value = ie.get_config(deviceName, ov::enable_profiling));
    ASSERT_TRUE(value);
}
...
TEST(OVClassBasicTest, smoke_GetMetricSupportedMetricsHeteroNoThrow) {
    ...
    OV_ASSERT_NO_THROW(t = ie.get_metric(deviceName, ov::supported_metrics));
    ...
    ASSERT_METRIC_SUPPORTED(ov::supported_metrics);
}
...
TEST(OVClassBasicTest, smoke_GetMetricSupportedConfigKeysHeteroNoThrow) {
    ...
    OV_ASSERT_NO_THROW(t = ie.get_metric(deviceName, ov::supported_config_keys));
    ...
    ASSERT_METRIC_SUPPORTED(ov::supported_config_keys);
}
...
TEST(OVClassBasicTest, smoke_GetMetricSupportedConfigKeysHeteroThrows) {
    ...
    ASSERT_THROW(ie.get_metric(targetDevice, ov::supported_config_keys), ov::Exception);
}
...
TEST_P(OVClassGetMetricTest_AVAILABLE_DEVICES, GetMetricAndPrintNoThrow) {
    ...
    OV_ASSERT_NO_THROW(t = ie.get_metric(deviceName, ov::available_devices));
    ...
    ASSERT_METRIC_SUPPORTED(ov::available_devices);
}
...
TEST_P(OVClassGetMetricTest_FULL_DEVICE_NAME, GetMetricAndPrintNoThrow) {
    ...
    OV_ASSERT_NO_THROW(t = ie.get_metric(deviceName, ov::device::full_name));
    std::cout << "Full device name: " << std::endl << t << std::endl;

    ASSERT_METRIC_SUPPORTED(ov::device::full_name);
}

TEST_P(OVClassGetMetricTest_OPTIMIZATION_CAPABILITIES, GetMetricAndPrintNoThrow) {
    ov::runtime::Core ie = createCoreWithTemplate();
    std::vector<std::string> t;
    OV_ASSERT_NO_THROW(t = ie.get_metric(deviceName, ov::optimization_capabilities));
...
TEST_P(OVClassGetMetricTest_DEVICE_GOPS, GetMetricAndPrintNoThrow) {
    ...
    for (auto&& kv : ie.get_metric(deviceName, ov::device::gops)) {
        std::cout << kv.first << ": " << kv.second << std::endl;
    }
    ASSERT_METRIC_SUPPORTED(ov::device::gops);
}

TEST_P(OVClassGetMetricTest_DEVICE_TYPE, GetMetricAndPrintNoThrow) {
    ov::runtime::Core ie = createCoreWithTemplate();
    ASSERT_METRIC_SUPPORTED(ov::device::type);
    ov::device::Type t = {};
    OV_ASSERT_NO_THROW(t = ie.get_metric(deviceName, ov::device::type));
...
TEST_P(OVClassGetMetricTest_ThrowUnsupported, GetMetricThrow) {
    ...
    ASSERT_THROW(ie.get_metric(deviceName, "unsupported_metric"), ov::Exception);
}
...
TEST_P(OVClassGetConfigTest, GetConfigNoThrow) {
    ...
    OV_ASSERT_NO_THROW(configValues = ie.get_metric(deviceName, ov::supported_config_keys));

    for (auto&& confKey : configValues) {
        ov::Any defaultValue;
        OV_ASSERT_NO_THROW(defaultValue = ie.get_config(deviceName, confKey));
...
TEST_P(OVClassGetConfigTest, GetConfigHeteroNoThrow) {
    ...
    for (auto&& confKey : configValues) {
        OV_ASSERT_NO_THROW(ie.get_config(deviceName, confKey));
    }
}

TEST_P(OVClassGetConfigTest_ThrowUnsupported, GetConfigHeteroThrow) {
    ov::runtime::Core ie = createCoreWithTemplate();
    ASSERT_THROW(ie.get_config(CommonTestUtils::DEVICE_HETERO, "unsupported_config"), ov::Exception);
}

TEST_P(OVClassGetConfigTest_ThrowUnsupported, GetConfigHeteroWithDeviceThrow) {
    ov::runtime::Core ie = createCoreWithTemplate();

    ASSERT_THROW(ie.get_config(CommonTestUtils::DEVICE_HETERO + std::string(":") + deviceName,
                                   ov::dump_graph_dot),
                 ov::Exception);
}
...
TEST_P(OVClassGetConfigTest_ThrowUnsupported, GetConfigThrow) {
    ...
    ASSERT_THROW(ie.get_config(deviceName, "unsupported_config"), ov::Exception);
}
...
TEST_P(OVClassQueryNetworkTest, QueryNetworkHETEROWithDeviceIDNoThrow) {
    ...
    if (supportsDeviceID(ie, deviceName)) {
        auto deviceIDs = ie.get_metric(deviceName, ov::available_devices);
        if (deviceIDs.empty())
            GTEST_SKIP();
...
TEST_P(OVClassLoadNetworkTest, LoadNetworkHETEROwithMULTINoThrow) {
    ...
    if (supportsDeviceID(ie, deviceName) && supportsAvaliableDevices(ie, deviceName)) {
        std::string devices;
        auto availableDevices = ie.get_metric(deviceName, ov::available_devices);
        ...
        OV_ASSERT_NO_THROW(
            ie.compile_model(actualNetwork,
                             CommonTestUtils::DEVICE_HETERO,
                             ov::device::config(CommonTestUtils::DEVICE_MULTI,
                                               ov::device::priorities(devices)),
                             ov::device::config(CommonTestUtils::DEVICE_HETERO,
...
TEST_P(OVClassLoadNetworkTest, LoadNetworkMULTIwithHETERONoThrow) {
    ...
    if (supportsDeviceID(ie, deviceName) && supportsAvaliableDevices(ie, deviceName)) {
        ...
        OV_ASSERT_NO_THROW(ie.compile_model(
            ...
            ov::device::config(CommonTestUtils::DEVICE_MULTI, ov::device::priorities(devices)),
            ov::device::config(CommonTestUtils::DEVICE_HETERO, ov::target_fallback(deviceName, deviceName))));
    } else {
        GTEST_SKIP();
    }
}
...
TEST_P(OVClassSetDefaultDeviceIDTest, SetDefaultDeviceIDNoThrow) {
    ...
    std::string value;
    OV_ASSERT_NO_THROW(ie.set_config(deviceName, ov::device::id(deviceID), ov::enable_profiling(true)));
    ASSERT_TRUE(ie.get_config(deviceName, ov::enable_profiling));
    OV_ASSERT_NO_THROW(value = ie.get_config(deviceName, ov::enable_profiling.str()).as<std::string>());
    ASSERT_EQ(value, "YES");
}
...
TEST_P(OVClassSetGlobalConfigTest, SetGlobalConfigNoThrow) {
    ...
    for (auto& dev_id : deviceIDs) {
        OV_ASSERT_NO_THROW(ie.set_config(deviceName + "." + dev_id, ov::enable_profiling(false)));
    }
    OV_ASSERT_NO_THROW(ie.set_config(deviceName, ov::enable_profiling(true)));
    OV_ASSERT_NO_THROW(ref = ie.get_config(deviceName, ov::enable_profiling.str()));

    for (auto& dev_id : deviceIDs) {
        OV_ASSERT_NO_THROW(src = ie.get_config(deviceName + "." + dev_id, ov::enable_profiling.str()));
        ASSERT_EQ(src, ref);
    }
}
...
TEST_P(OVClassSeveralDevicesTestDefaultCore, DefaultCoreSeveralDevicesNoThrow) {
    ...
    for (size_t i = 0; i < deviceNames.size(); ++i) {
        OV_ASSERT_NO_THROW(ie.set_config(deviceNames[i], ov::enable_profiling(true)));
    }
    bool res;
    for (size_t i = 0; i < deviceNames.size(); ++i) {
        OV_ASSERT_NO_THROW(res = ie.get_config(deviceNames[i], ov::enable_profiling));
        ASSERT_TRUE(res);
    }
}
}  // namespace behavior
