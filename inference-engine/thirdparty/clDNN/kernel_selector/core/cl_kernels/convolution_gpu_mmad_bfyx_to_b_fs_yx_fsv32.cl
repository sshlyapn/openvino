...
#endif
                for (int xb = 0; xb < INPUT_LINE_SIZE; xb++) {
                    line_cache[yb * INPUT_LINE_SIZE + xb] = slm[kd*SLM_LINE_SIZE*INPUT_Y_HEIGHT + ((kh*DILATION_SIZE_Y)+yb*STRIDE_SIZE_Y)*SLM_LINE_SIZE + sg*OUTPUT_X_BLOCK_SIZE*STRIDE_SIZE_X + xb];
                }
            }
#else
            bool y_cross_fm = input_y + kh*DILATION_SIZE_Y < 0 || input_y + kh*DILATION_SIZE_Y >= INPUT0_SIZE_Y;
