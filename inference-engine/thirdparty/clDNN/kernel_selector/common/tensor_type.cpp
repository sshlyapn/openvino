...
WeightsTensor::WeightsChannelArray WeightsTensor::weightsChannelArray {
    ...
    { WeightsLayout::os_is_yx_isv16_osv16,                        {{  0,  1, -1,   2,   3, -1, -1, -1 }, {{WeightsChannelName::OFM, 16}, {WeightsChannelName::IFM, 16}} } },
...
DataTensor DataTensor::FlattenFeatureAndSpatials() const {
    ...
    if (l == DataLayout::bf) {
        res.dims[Channelndex(l, DataChannelName::BATCH)].pitches[0] = { b.Pitch(), 1 };
        res.dims[Channelndex(l, DataChannelName::BATCH)].pad = b.pad;
    } else {
        res.dims[Channelndex(l, DataChannelName::FEATURE)].pitches[0] = { dims[Channelndex(l, DataChannelName::BATCH) + 1].Pitch(), 1 };
...
NDims WeightsTensor::GetSimpleDims(const std::vector<size_t>& d, WeightsLayout l) {
    std::vector<size_t> newDims = d; // block_desc newDims = [block_1, block_2 .. block_N, X, Y, Z, IFM, IFM, LY, LX, G]
                                     // block_desc size is = d.size() + blocks.size()

    auto blockedChannels = GetBlocked(l);

    printf("BlockedChannels Size: %lu\n", blockedChannels.size());
    ...
    if (l == i_yxs_os_yxsv2_osv16) {
        ret[3].pitches = {{RoundUp(ret[1].v * ret[2].v, 2) * ret[1].Pitch(), 1}};
        ret[2].pad.after = newDims[2] - ret[2].v;
    } else if (l == iy_xs_os_xsv2_osv16__ao32 ||
               l == iy_xs_os_xsv2_osv8__ao32) {
        ret[2].pitches = {{RoundUp(ret[1].v, 2) * ret[1].Pitch(), 1}};
        ret[1].pad.after = newDims[1] - ret[1].v;

        ret[3].pitches = {{ret[2].v * ret[2].Pitch(), 1}};
        ret[2].pad.after = newDims[2] - ret[2].v;
    } else if (l == os_is_yx_isa8_osv8_isv4 || l == os_is_yx_isa8_osv8_isv4_swizzled_by_4) {
        ret[0].pitches = {{256, 1}};
        ret[1].pitches = {{ret[0].Pitch() * ret[0].v, 1}};
    } else if (l == os_i_yxs_osv4_yxsv4) {
        ret[2].pitches = {{RoundUp(ret[0].v * ret[1].v, 4) * 4, 1}};
        ret[3].pitches = {{ret[2].v * RoundUp(ret[0].v * ret[1].v, 4), 1}};
    } else if (l == gi_yxs_os_yxsv2_osv16) {
        ret[3].pitches = {{RoundUp(ret[1].v * ret[2].v, 2) * ret[1].Pitch(), 1}};
        ret[2].pad.after = newDims[2] - ret[2].v;
        ret[4].pitches = {{ret[3].Pitch() * ret[3].v, 1}};
        ...
        ret[2].pitches = {{RoundUp(ret[0].v * ret[1].v, 4) * 4, 1}};
        ret[4].pitches = {{ret[3].v * RoundUp(ret[0].v * ret[1].v, 4), 1}};
    } else if (l == gs_oi_yxs_gsv16_yxsv4) {
        ret[2].pitches = {{RoundUp(ret[0].v * ret[1].v, 4) * 16, 1}};
        ret[4].pitches = {{ret[3].v * RoundUp(ret[0].v * ret[1].v, 4), 1}};
    } else if (l == gs_oi_yxs_gsv32_yxsv4) {
        ret[2].pitches = {{RoundUp(ret[0].v * ret[1].v, 4) * 32, 1}};
        ret[4].pitches = {{ret[3].v * RoundUp(ret[0].v * ret[1].v, 4), 1}};
    }
    return ret;
}
...
WeightsTensor WeightsTensor::TransformIgnorePadding(WeightsLayout l, WeightsType t, size_t g, bool should_split) const {
    ...
    const auto simple_channels_num = ChannelsCount(l);
    const auto blocked_channels_num = BlockedChannelsCount(l);
    // const auto blocked_channels = GetBlocked(l);

    std::vector<size_t> vec(simple_channels_num);
    std::vector<size_t> vecBlocked(simple_channels_num + blocked_channels_num);

    // auto findBlockedChannel = [&](WeightsChannelName channel) {
    //     auto total_channel_size = 1;
    //     for (auto& blockedChannel : blocked_channels) {
    //         if (blockedChannel.first == channel) {
    //             total_channel_size *= blockedChannel.second;
    //         }
    //     }
    //     return total_channel_size;
    // };

    // for (size_t i = 0; i < blocked_channels_num; i++) {
    //     vecBlocked[i] = blocked_channels[i].second;
    // }
    ...
    // for (auto channelName : WeightsChannelNames) {
    //     const auto channelIndex = Channelndex(l, channelName);
    //     if (channelIndex != -1) {
    //         vecBlocked[blocked_channels_num + channelIndex] = vec[channelIndex] / findBlockedChannel(channelName);
    //     }
    // }

    printf("Weight Transformed: %lu vs %lu and %d\n", vec.size(), vecBlocked.size(), blocked_channels_num);
